...
let API_URL = "https://pointer-dev.cs.stanford.edu/cs193x_api/jackyuan";
...
const apiRequest = async (method, path, body = null) => {
  let response = null;
  if (body === null){
    response = await fetch(API_URL + path, {method: method,headers: {"Content-Type": "application/json"}}); 
  } else {
    response = await fetch(API_URL + path, {method: method,headers: {"Content-Type": "application/json"},
  ...
  if (response.status === 200) {
    ...
    //throw new Error("Error occured with error code: " + JSON.stringify(response.status));
